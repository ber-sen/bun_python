...
export class PyObject {
    ...
    switch (typeof v) {
      ...
      case "object": {
        if (v === null) {
          ...
          view.setBigUint64(0, BigInt(namePtr), LE);
          ...
          for (const [key, value] of v instanceof Map
            ? v.entries()
            : Object.entries(v)) {
   ...
   */
  asString() {
    return py.PyUnicode_AsUTF8(this.handle) + "";
  }
  ...
  *[Symbol.iterator]() {
  ...
  call(
    ...
    if (positionalCount < 0) {
      throw new TypeError(
        `${this.toString()} requires at least ${namedCount} arguments, but only ${
          positional.length
        } were passed`
      );
    }
